 ...
 */
inline fun <T: Any> Observable<T>.doOnNextFx(crossinline onNext: (T) -> Unit): Observable<T> = doOnNext {
 ...
 * Performs the provided onNext action on the FX thread
 */
inline fun <T: Any> Flowable<T>.doOnNextFx(crossinline onNext: (T) -> Unit): Flowable<T> = doOnNext {
    Platform.runLater { onNext.invoke(it) }
}
 ...
 */
inline fun <T: Any> Observable<T>.doOnErrorFx(crossinline onError: (Throwable) -> Unit): Observable<T> = doOnError {
 ...
 * Performs the provided onError action on the FX thread
 */
inline fun <T: Any> Flowable<T>.doOnErrorFx(crossinline onError: (Throwable) -> Unit): Flowable<T> = doOnError {
    Platform.runLater { onError.invoke(it) }
}
 ...
 */
inline fun <T: Any> Observable<T>.doOnCompleteFx(crossinline onComplete: () -> Unit): Observable<T> = doOnComplete {
 ...
 * Performs the provided onComplete action on the FX thread
 */
inline fun <T: Any> Flowable<T>.doOnCompleteFx(crossinline onComplete: () -> Unit): Flowable<T> = doOnComplete {
    Platform.runLater { onComplete.invoke() }
}
 ...
 */
inline fun <T: Any> Observable<T>.doOnSubscribeFx(crossinline onSubscribe: () -> Unit): Observable<T> = doOnSubscribe {
 ...
 * Performs the provided onSubscribe action on the FX thread
 */
inline fun <T: Any> Flowable<T>.doOnSubscribeFx(crossinline onSubscribe: () -> Unit): Flowable<T> = doOnSubscribe {
    Platform.runLater { onSubscribe.invoke() }
}
 ...
 */
inline fun <T: Any> Observable<T>.doOnTerminateFx(crossinline onTerminate: () -> Unit): Observable<T> = doOnTerminate {
 ...
 * Performs the provided onTerminate action on the FX thread
 */
inline fun <T: Any> Flowable<T>.doOnTerminateFx(crossinline onTerminate: () -> Unit): Flowable<T> = doOnTerminate {
    Platform.runLater { onTerminate.invoke() }
}
 ...
 */
inline fun <T: Any> Observable<T>.doOnDisposeFx(crossinline onDispose: () -> Unit): Observable<T> = this.doOnDispose {
 ...
 * Performs the provided onDispose action on the FX thread
 */
inline fun <T: Any> Flowable<T>.doOnCancelFx(crossinline onDispose: () -> Unit): Flowable<T> = this.doOnCancel {
    Platform.runLater { onDispose.invoke() }
}
 ...
 */
fun <T: Any> Observable<T>.doOnNextCount(onNext: (Int) -> Unit): Observable<in T> =
 ...
 * Executes side effect with the accumulating count of emissions for each onNext() call
 */
fun <T: Any> Flowable<T>.doOnNextCount(onNext: (Int) -> Unit): Flowable<in T> =
 ...
 */
fun <T: Any> Observable<T>.doOnCompleteCount(onComplete: (Int) -> Unit): Observable<in T> =
        compose(FxObservableTransformers.doOnCompleteCount(onComplete))

/**
 * Executes side effect with the total count of emissions for the onComplete() call
 */
fun <T: Any> Flowable<T>.doOnCompleteCount(onComplete: (Int) -> Unit): Flowable<in T> =
 ...
 */
fun <T: Any> Observable<T>.doOnErrorCount(onError: (Int) -> Unit): Observable<in T> =
        compose(FxObservableTransformers.doOnErrorCount(onError))

/**
 * Executes side effect with the total count of emissions for an onError() call
 */
fun <T: Any> Flowable<T>.doOnErrorCount(onError: (Int) -> Unit): Flowable<in T> =
 ...
 */
fun <T: Any> Observable<T>.doOnNextCountFx(onNext: (Int) -> Unit) = doOnNextCount { Platform.runLater { onNext.invoke(it) } }

/**
 * Executes side effect on FX thread with the accumulating count of emissions for each onNext() call
 */
fun <T: Any> Flowable<T>.doOnNextCountFx(onNext: (Int) -> Unit) = doOnNextCount { Platform.runLater { onNext.invoke(it) } }

/**
 * Executes side effect on FX thread with the total count of emissions for the onComplete() call
 */
fun <T: Any> Observable<T>.doOnCompleteCountFx(onComplete: (Int) -> Unit) = doOnCompleteCount { Platform.runLater { onComplete.invoke(it) } }

/**
 * Executes side effect on FX thread with the total count of emissions for the onComplete() call
 */
fun <T: Any> Flowable<T>.doOnCompleteCountFx(onComplete: (Int) -> Unit) = doOnCompleteCount { Platform.runLater { onComplete.invoke(it) } }

/**
 * Executes side effect on FX thread with the total count of emissions for the onError() call
 */
fun <T: Any> Observable<T>.doOnErrorCountFx(onError: (Int) -> Unit) = doOnErrorCount { Platform.runLater { onError.invoke(it) } }


/**
 * Executes side effect on FX thread with the total count of emissions for the onError() call
 */
fun <T: Any> Flowable<T>.doOnErrorCountFx(onError: (Int) -> Unit) = doOnErrorCount { Platform.runLater { onError.invoke(it) } }
